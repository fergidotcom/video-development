#!/usr/bin/env python3
"""
Ferguson Family Archive Transcription Pipeline - LOCAL VERSION

Transcribes audio from LOCAL Mac storage (no Pegasus dependency).
...
- LOCAL: Reads audio from ~/Documents/VideoTranscripts/ExtractedAudio/
"""
...
LOCAL_AUDIO_BASE = os.path.expanduser("~/Documents/VideoTranscripts/ExtractedAudio")
SURVEY_DB = os.path.expanduser("~/Library/CloudStorage/Dropbox/Fergi/VideoDev/pegasus-survey.db")
TRANSCRIPTS_DB = os.path.expanduser("~/Documents/VideoTranscripts/transcripts.db")  # LOCAL DB
LOG_FILE = "/tmp/transcribe-ferguson-local.log"
...
def convert_pegasus_to_local_path(pegasus_path):
    """Convert Pegasus path to local Mac path."""
    pegasus_base = "/Volumes/Promise Pegasus/ExtractedAudio"
    if pegasus_path.startswith(pegasus_base):
        rel_path = pegasus_path[len(pegasus_base)+1:]  # +1 for the trailing slash
        local_path = os.path.join(LOCAL_AUDIO_BASE, rel_path)
        return local_path
    return pegasus_path


def get_ferguson_files_to_transcribe():
    """Query Ferguson files with extracted audio, ordered by duration."""
...
def main():
    logger.info("=" * 80)
    logger.info("🎬 Ferguson Family Archive Transcription Pipeline - LOCAL MODE")
    logger.info(f"   Reading audio from: {LOCAL_AUDIO_BASE}")
    logger.info(f"   Database: {TRANSCRIPTS_DB}")
    logger.info("=" * 80)
    ...
    for file_record in files_to_process:
        ...
        pegasus_audio_path = file_record['audio_path']

        # CONVERT TO LOCAL PATH
        audio_path = convert_pegasus_to_local_path(pegasus_audio_path)
        ...
        logger.info(f"   Audio: {os.path.basename(audio_path)} [LOCAL]")
        ...
        if not os.path.exists(audio_path):
            ...
            mark_transcription_error(file_id, f"Local audio file not found: {audio_path}")
            continue
    ...
    # Final summary
    logger.info("\n" + "=" * 80)
    logger.info("🏁 TRANSCRIPTION SESSION COMPLETE - LOCAL MODE")
    logger.info("=" * 80)
